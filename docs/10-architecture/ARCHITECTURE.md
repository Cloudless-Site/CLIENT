...
1. Control plane:
   SSH commands such as register@, verify@, put@, get@, protect@, activate@, vendor@, and admin commands.
...
Transport protocol such as http, https, tcp, or udp is derived from the reverse bind specification.

HTTPS routing may operate in passthrough or proxy mode depending on backend detection and runtime conditions.
